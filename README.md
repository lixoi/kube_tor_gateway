...
Необходимость Chart-а обусловлена описанием конфигурации сетевых интерфейсов клиентских узлов цепочки в соответствии с конфигурацией физической сети кластера, а также заданием параметров каждого узла цепочки на основе CRD TorChain:
...
Перед инициализацией VPN-цепочки необходимо:
 - На сервисе Vault:
   ...
   3. Создать роль на доступ к секретам (read) и привязать ее к сервис-аккаунту пользователя, который будет создавать цепочку, а так же привязать к namespace, в котором будет развернута цепочка (Release.Name в helm chart!).
   4. Записать необходимое количество параметров доступа к серверам. Например:
		- vault kv put kvv2/ns/number-node-chain/domain1/city1/server1/wg client.vpn="creds-in-base64"
		- vault kv put kvv2/ns/number-node-chain/domain2/city2/server2/ovpn client.vpn="creds-in-base64"
	...
	Ключ строго фиксирован (client.vpn), так как по имени ключа создается конфигурационный файл. Данное имя задано в коде оператора.
 - В кластере:
	1. Инициировать аутентификацию для секрета. Например:
        ```
			...
			spec:
    		  ...
    		  audiences:
      		    - vault
        ```
	2. Создать ресурсы VaultStaticSecret, которые монтирую конфигурационные параметры доступа (п.4) к имени секрета (label). Например:
        ```
			apiVersion: secrets.hashicorp.com/v1beta1
        ...
        ```
Таким образом, в переменной кастомного ресурса tor-оператора VpnSecretNames задается список параметров доступа к серверам (из ресурсов VaultStaticSecret), на которые можно переключаться конкретному узлу цепочки. 
...
В качестве системы сбора логов был выбран стек: fluent-bit - loki - gragana.

OpenVPN клиент отправляет логи в стандартные потоки вывода: stdout/stderr. Таким образом, для openvpn необходио настроить сбор логов из контейнера НОД-ы. Например, в ресурсе ConfigMap агента fluent-bit необходимо настроить правило:
```
[INPUT]
	Name              tail
	Tag               kube.*
	...
	Buffer_Max_size   32k
```
Wireguard клиент по умолчанию не ведет логирование, так как запускается как драйвер ядра ОС.
Возможно запускать wireguard в user-mode режиме, тогда клиент будет логировать события в стандартные потоки вывода (по аналогии с OpenVPN).

В образе (Docker/wguard-build-image/default.nix) wireguard инсталлируется как стандартный пакет и запускается по умочанию (как драйвер). В таком случае, существует три способа получения логов: 
1. DynDBG - запуск wireguard в debug-режиме: wg0 +p 
2. Tcpdump - фильтрация сетевых пакетов сниффером: tcpdump -ttttni any 'udp port 51820'
3. Iptables - настройка сетевого фильтра на логирование пакетов wireguard: 
...
Первый вариант влияет на производительность сервиса и является плохой практикой. Второй вариант требует наличие в ПОДе сниффера Tcpdump и его запуска. Так как в wireguard-контейнере установлен iptables для задания правил трансляции сетевых пакетов, то был выбран третий способ. Для получения логов iptables требуется настроить fluent-bit агент на логирование системных логов ядра НОДы, например:
```
[INPUT]
	Name              syslog
	Tag               kern.*
	...
	Buffer_Max_size   32k
```
И фильтровать (в grafana) по префиксу wireguard iptables.
